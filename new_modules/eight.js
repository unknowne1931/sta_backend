...
// ----------------------------------------------
// COUNT ONLY BROKEN CIRCLE BOXES ⭕💥
// ----------------------------------------------
export function countBrokenCircles(boxes) {
  return boxes.filter(
...
function drawBoxBorderWithGap(ctx, x, y, size, broken) {
  ...
  // draw ONLY the broken segment in white
  if (broken) {
...
export function drawImage(boxes) {
  ...
  boxes.forEach(b => {
    ...
    boxes.forEach(b => {
      ...
      // random color for broken shapes too
      const strokeColor = b.complete ? b.color : randomColor();
