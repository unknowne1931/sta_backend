...
// ----------------------------------------------
// COUNT COMPLETE CIRCLES + TRIANGLES
// (in different boxes)
// ----------------------------------------------
export function countCompleteCirclesAndTriangles(boxes) {
  const completeCircles = boxes.filter(
    b => b.complete && b.shape === "circle"
  ).length;

  const completeTriangles = boxes.filter(
    b => b.complete && b.shape === "triangle"
  ).length;

  return completeCircles + completeTriangles;
}
...
// ----------------------------------------------
export function generatePuzzle_complete_nine(per) {
  const difficulty = getDifficultiesByPer(per);
  const boxes = generateBoxesData(difficulty);

  const correct = countCompleteCirclesAndTriangles(boxes);
  const options = generateOptions(correct);
  const image = drawImage(boxes);

  return {
    question:
      "How many complete boxes contain circles and triangles (in different boxes)?",
