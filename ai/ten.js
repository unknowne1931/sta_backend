...
// HELPERS
// ==========================
const getIdx = (x, y, cols, rows) => (x < 0 || y < 0 || x >= cols || y >= rows) ? -1 : x + y * cols;
...
export function generateMazeQuestion({ level = "Medium" ,per = 50 } = {}) {
    ...
    const grid = Array.from({ length: rows * cols }, (_, i) => new Cell(i % cols, Math.floor(i / cols)));

    // 1. Standard Maze Generation (Iterative DFS)
    const stack = [];
    let current = grid[0];
    ...
    // 2. Define Start (Blue) and End (Red)
    const start = { x: 0, y: Math.floor(Math.random() * rows) };
    const end = { x: cols - 1, y: Math.floor(Math.random() * rows) };

    // 3. Randomly make it impossible by closing a few walls
    const makeImpossible = Math.random() < 0.5;
    if (makeImpossible) {
        // We close 2-4 random horizontal/vertical segments to block paths
        for (let i = 0; i < 4; i++) {
            const target = grid[Math.floor(Math.random() * grid.length)];
            target.walls.left = target.walls.right = true;
        }
    }
    ...
    ctx.beginPath();
    ctx.arc(start.x * cellSize + offset, start.y * cellSize + offset, radius, 0, Math.PI * 2);
    ctx.fill();

    // Red (End)
    ctx.fillStyle = "#E74C3C";
    ctx.beginPath();
    ctx.arc(end.x * cellSize + offset, end.y * cellSize + offset, radius, 0, Math.PI * 2);
    ctx.fill();
