...
app.post('/get/balance/new/data', async (req, res) => {
...
app.post("/bank/upi/data/collect", authMiddleware, async (req, res) => {
    const { ac_h_nme, bank_nme, Acc_no, ifsc, app, type } = req.body;
    const user = req.user;
    try {
...
app.post("/get/id/to/update/seonds", authMiddleware, async (req, res) => {
    const { id, sec, qst, a, b, c, d, img, ans, usa, vr, msg, ex_seconds, cat, tough } = req.body;
    const user = req.user;
...
app.post("/get/posted/count/questions", authMiddleware, async (req, res) => {
    const { img, Questio, a, b, c, d, Ans, lang, tough, seconds } = req.body;
    const user = req.user;
...
app.post("/get/new/cupon/for/neww/cupon", authMiddleware, async (req, res) => {
    const { title, img, valid, body, type } = req.body;
    const user = req.user;
    try {
...
app.get("/get/language/datas/all/get",authMiddleware , async (req, res) => {
    const user = req.user;
    try {
        if (!user) return res.status(400).json({ Status: "BAD", message: "Some Data Missing" })
        const Users = await LanguageSelectModule.findOne({ user }).lean()
...
app.post("/refer/and/earn", authMiddleware , async (req, res) => {
    const { referd_user_d_id, referd } = req.body;
    const user = req.user;
    try {
